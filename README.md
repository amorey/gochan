...
`RecvContext` uses the same precedence: **closed > cancelled > value**. `ErrClosed` wins whenever the receive is terminal (the receiver or hub is closed), or the sender has closed and the buffer is empty.

If a value and a sender-close arrive together while parked, every channel type delivers the value first and `ErrClosed` only follows once nothing is left.

If a value and a receiver-close or hub-close arrive together while parked:

* `spsc`, `spmc`, `mpsc`, `mpmc` — parked on a `select` whose arms include the value channel alongside close, so they resolve at random. A value that wins has already been dequeued and is returned rather than thrown away and the next call reports `ErrClosed`.
* `broadcast`, `watch` — parked on a wakeup that carries no value, so nothing is consumed on wake. The loop re-checks closed before reading the slot or ring, so against a pending value `ErrClosed` always wins.
* `oneshot` — a `Send` that committed before the close wins and is returned, otherwise `ErrClosed`.

If a value and a cancellation arrive together while parked:

* `spsc`, `spmc`, `mpsc`, `mpmc` — parked directly on the value channel, so they resolve at random like any `select`. A value that wins has already been dequeued and is returned rather than thrown away.
* `broadcast`, `watch` — parked on a wakeup that carries no value, so nothing is consumed on wake. The loop re-checks closed and `ctx` before reading the slot or ring, so against a pending value the full closed > cancelled precedence holds.
* `oneshot` — parked on the pair's termination signal, so it resolves at random but the slot is re-read under a lock afterwards, so a cancellation that wins leaves the value for a later `Recv`.
...
"Abandoned" above describes a close already visible when `Recv` is entered; a close landing *during* an in-flight `Recv` is ordered the other way, per [Errors](#errors). Either way no value is consumed and discarded — it is delivered to the caller or left in the shared queue for the remaining live receivers. Which side of that boundary a racing close falls on is a race; don't depend on it.

The send side has a matching boundary, and it resolves toward the buffer. On the queue-style packages a `Send` / `TrySend` / `SendContext` racing the close of the **last** receiver may return `nil` for a value nothing will ever drain — the window between confirming the pipeline is live and landing the value cannot be closed without deadlocking teardown against the sender. So a successful send means "the queue accepted this value," not "a receiver will consume it" — the same guarantee a plain Go buffered channel gives when its last reader goes away. Producers that need delivery confirmed should acknowledge on a return path rather than infer it from a `nil` error.
