...
// receiver is already blocked in the parking select when the value and
// the cancellation both land, so the wake races between the <-notify and
// <-ctxDone arms. Either way the next loop iteration re-derives the
// answer from state and must return ctx.Err() with the value still in
// the ring: waking consumes nothing, and the loop-top ctx check sits
// above the read.
func TestRecvContextCancelBeatsValueDeliveredWhileParked(t *testing.T) {
	...
	// parked receiver is woken by <-notify with a value genuinely ready
	// and only then observes the cancellation. Sending first and
	...
	v, err := rx.TryRecv()
	require.NoError(t, err)
	assert.Equal(t, 7, v, "value consumed by a cancelled parked receive")
}
